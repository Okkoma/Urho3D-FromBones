...
bool SplitPacketSort::Add(InternalPacket *internalPacket)
{
	...
	RakAssert(m_data[internalPacket->splitPacketIndex] == 0);

	if (m_data[internalPacket->splitPacketIndex] == 0) {
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::ApplyNetworkSimulator( float _packetloss, SLNet::TimeMS _minExtraPing, SLNet::TimeMS _extraPingVariance )
{
#ifdef _RELEASE
	// unused parameters
	(void)_packetloss;
