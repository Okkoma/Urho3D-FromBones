...
void GetMyIP_Windows(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS], const ULONG addressFamily)
{
	...
	DWORD error = GetAdaptersAddresses(addressFamily, GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_FRIENDLY_NAME, 0, pAddresses, &outBufLen);
	...
	while ((pCurAdapter != 0) && (outAddressIndex < MAXIMUM_NUMBER_OF_INTERNAL_IDS)) {
