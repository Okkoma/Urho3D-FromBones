...
static VideoBootStrap *bootstrap[] = {
    ...
    &COCOA_bootstrap,
#endif
// C.VILLE - RPI Legacy : Prior the RPI Video over X11 for RPIVIDEO in X11 Window
...
//    SDL_Log("Try Driver %s", driver_name);
...
//    SDL_Log("Use Driver %s", driver_name);
...
SDL_UpdateFullscreenMode(SDL_Window * window, SDL_bool fullscreen)
{
    ...
    */
    ...
    // Urho3D C.VILLE : remove this because fullscreen can be made with a different displaymode
...
void
SDL_OnWindowPositionChanged(SDL_Window * window)
{
    window->surface_valid = SDL_FALSE;
...
// TEST FOR RPI EGL-X11 (CVILLE)
void
SDL_OnWindowBeginConfigure(SDL_Window * window)
{
#ifdef RPI
    if (_this->OnWindowBeginConfigure) {
        _this->OnWindowBeginConfigure(_this, window);
    }
#endif
}
...
{
#ifdef RPI
	if (_this->OnWindowLeave) {
        _this->OnWindowLeave(_this, window);
    }
#endif
}
