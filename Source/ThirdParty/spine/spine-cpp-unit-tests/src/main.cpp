...
void loadBinary(const spString &binaryFile, const spString &atlasFile, Atlas *&atlas, SkeletonData *&skeletonData,
...
void loadJson(const spString &jsonFile, const spString &atlasFile, Atlas *&atlas, SkeletonData *&skeletonData,
			  AnimationStateData *&stateData, Skeleton *&skeleton, AnimationState *&state) {
	atlas = new (__FILE__, __LINE__) Atlas(atlasFile, NULL);
	assert(atlas != NULL);
...
struct TestData {
	TestData(const spString &jsonSkeleton, const spString &binarySkeleton, const spString &atlas) : _jsonSkeleton(
																									  jsonSkeleton),
																							  _binarySkeleton(
																									  binarySkeleton),
																							  _atlas(atlas) {}

	spString _jsonSkeleton;
	spString _binarySkeleton;
	spString _atlas;
};

void testLoading() {
	spVector<TestData> testData;
